// Package awsretry は AWS API 呼び出し向けのリトライユーティリティを提供する。
//
// DynamoDB の ProvisionedThroughputExceededException や S3 の SlowDown など、
// 「時間をおけば成功しうる」一時的な失敗に対して、
// AWS 公式推奨の「指数バックオフ + フルジッター」方式で自動リトライする。
//
// processor / scheduler の両 Lambda から共有するため internal パッケージに置く。
// 同リポジトリの Python 版・TypeScript 版と同じ設計思想の並置実装。
//
// 設計方針:
...
// 注意:
//
//	リトライ対象の呼び出しが io.Reader をボディに取る場合（S3 PutObject 等）、
//	Reader は初回で消費されるため、必ずクロージャの内側で作り直すこと。
package awsretry
